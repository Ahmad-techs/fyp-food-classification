...
def _eval_single(model, loader, device):
    """Evaluate in single-head mode. Model returns (fine_out, coarse_out) — unpack it."""
    model.eval()
    correct = total = 0
    for imgs, fl, _ in loader:
        imgs, fl   = imgs.to(device), fl.to(device)
        f_out, _   = model(imgs)          # ← FIX: unpack the tuple, ignore coarse
        correct   += (f_out.argmax(1) == fl).sum().item()
        total     += fl.size(0)
...
def _eval_dual(model, loader, device):
    """Evaluate in dual-head mode. Use both outputs."""
    model.eval()
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            if model_type == 'single':
                f_out, _ = model(imgs)            # ← FIX: unpack even in single mode
        ...
        if model_type == 'single':
            ...
            print(f'  {phase_name} Ep{epoch+1:02d}/{epochs} '
                  f'loss={avg_loss:.4f}  Val Top-1={top1:.2f}%')
        else:
            top1, ctop1 = _eval_dual(model, val_loader, device)
            print(f'  {phase_name} Ep{epoch+1:02d}/{epochs} '
                  f'loss={avg_loss:.4f}  Fine={top1:.2f}%  Coarse={ctop1:.2f}%')
...
def train_model(model, train_loader, val_loader, device,
    ...
    """
    3-phase training — IDENTICAL schedule for all 4 experimental conditions.
    model_type: 'single' for baseline, 'dual' for experiments 1-3.
    lam: coarse loss weight (0.5, 0.35, or 0.2). Ignored for single mode.
    """
    set_seeds(42)
    history = {}
    ...
    print('=== Phase 3: Full backbone unfrozen | 5 epochs | lr=5e-5 ===')
    model.unfreeze_all()
    history = _run_phase(model, train_loader, val_loader, device,
                         5, 5e-5, save_path, model_type, lam, patience, history, 'P3')

    print(f'\nTraining complete. Best val Top-1: {history["best_fine_acc"]:.2f}%')
    return history