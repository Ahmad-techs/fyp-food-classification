...
def _eval_single(model, loader, device):
    ...
    for imgs, fl, _ in loader:
        ...
        f_out, _ = model(imgs) # Unpack to ignore coarse
...
def train_model(model, train_loader, val_loader, device, save_path, model_type='single', lam=0.0, patience=5):
    ...
    print(f'=== Phase 3: Full backbone (5 epochs) ===')
    model.unfreeze_all()
